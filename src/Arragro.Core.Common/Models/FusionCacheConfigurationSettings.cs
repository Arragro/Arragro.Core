...
namespace Arragro.Core.Common.Models
{
    ...
    public class FusionCacheConfigurationSettings
    {
        ...
        public void Validate()
        {
            ...
            if (CacheType == FusionCacheType.Distributed && string.IsNullOrEmpty(CacheRedisConnection))
            ...
            if (CacheType == FusionCacheType.BackplaneAndDistributed && string.IsNullOrEmpty(CacheRedisConnection))
                throw new ArgumentException("If using a cache, you must supply a redis connection string.");
        }
    }
}
